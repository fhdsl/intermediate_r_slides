...
Say there is a `ggplot()` setting default that we never want to use. We can actually change the default arguments for this function. For example, we could write our own version of `geom_bar()`:

```{r}
library(ggplot2)
geom_my_bar <- function(...){
